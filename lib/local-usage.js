  ...
  return {
    async scan(now = Date.now()) {
      ...
      for (const filePath of paths) {
        ...
        // An append-only log that has not grown since the last scan has
        // nothing new to parse: keep the cached entry as is instead of deep
        // copying every event of every file on every refresh.
        if (previous && previous.offset > 0 && previous.offset === info.size) {
          continue;
        }
...
const DEFAULT_NOTE = 'in includes cache writes; cached in = cache reads; $ estimates API cost, not subscription billing';
const WINDOWS = [
...
export function formatModelTable(models, {
  ...
  const compactColumns = availableWidth < 50;
  const periodWidth = 6;
  const inputWidth = compactColumns ? 6 : 8;
  const cachedWidth = compactColumns ? 7 : 9;
  const outputWidth = compactColumns ? 6 : 8;
  const costWidth = compactColumns ? 7 : 10;
  const fixedWidth = periodWidth + inputWidth + cachedWidth + outputWidth + costWidth + 4;
  const modelWidth = Math.min(42, availableWidth - 2 - fixedWidth - 1);
  const inlineModel = modelWidth >= 6;
  const rows = [];
  const header = [
    ...(inlineModel ? [padVisible('model', modelWidth)] : []),
    padVisible('period', periodWidth),
    padStart('in', inputWidth),
    padStart(compactColumns ? 'cached' : 'cached in', cachedWidth),
    padStart('out', outputWidth),
    padStart('$', costWidth),
  ].join(' ');
  ...
  for (const entry of models) {
    ...
    if (!inlineModel) {
      rows.push(truncateTagged(
        `  {${rowTone}-fg}{bold}${escapeBlessed(shorten(entry.model))}{/bold}{/${rowTone}-fg}`,
        availableWidth,
      ));
    }

    windows.forEach((window, index) => {
      const usage = entry[window.key];
      const line = [
        ...(inlineModel ? [padVisible(index === 0 ? shorten(entry.model) : '', modelWidth)] : []),
        padVisible(window.label, periodWidth),
        padStart(formatTokens((usage.input || 0) + (usage.cacheWrite || 0)), inputWidth),
        padStart(formatTokens(usage.cacheRead || 0), cachedWidth),
        padStart(formatTokens(usage.output || 0), outputWidth),
        padStart(formatCost(usage), costWidth),
      ].join(' ');
      rows.push(`  {${rowTone}-fg}${escapeBlessed(line)}{/${rowTone}-fg}`);

      totals[window.key].input += usage.input || 0;
      totals[window.key].cacheRead += usage.cacheRead || 0;
      totals[window.key].cacheWrite += usage.cacheWrite || 0;
      totals[window.key].output += usage.output || 0;
      totals[window.key].cost += usage.cost || 0;
      totals[window.key].partial ||= Boolean(
        usage.hasUnknownCost
        || (usage.messages > 0 && !usage.hasCost),
      );
    });
  }

  if (!inlineModel) {
    rows.push('  {bold}total{/bold}');
  }

  windows.forEach((window, index) => {
    const total = totals[window.key];
    const totalLine = [
      ...(inlineModel ? [padVisible(index === 0 ? 'total' : '', modelWidth)] : []),
      padVisible(window.label, periodWidth),
      padStart(formatTokens(total.input + total.cacheWrite), inputWidth),
      padStart(formatTokens(total.cacheRead), cachedWidth),
      padStart(formatTokens(total.output), outputWidth),
      padStart(`${formatMoney(total.cost)}${total.partial ? '+?' : ''}`, costWidth),
    ].join(' ');
