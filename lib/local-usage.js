...
export function renderLocalUsage(local, { source = 'transcripts', ...tableOpts } = {}) {
  ...
  const header = `{cyan-fg}{bold}Local usage by model{/bold}{/cyan-fg} {white-fg}(${escapeBlessed(source)}, today / 7d / 30d / all time${fileCount}){/white-fg}`;
  const body = local.ok
    ? local.models.length > 0
      ? formatModelTable(local.models, tableOpts)
      : '  {white-fg}no local usage recorded{/white-fg}'
