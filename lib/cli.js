...
export async function runCli(argv = process.argv.slice(2)) {
  ...
  if (args.demo) {
    ...
    if (providers.some((provider) => ['claude', 'codex', 'gemini', 'kimi', 'zai'].includes(provider.id))) {
      const { initializeModelPricing } = await import('./model-pricing.js');
      await initializeModelPricing({ env });
    }
  }
