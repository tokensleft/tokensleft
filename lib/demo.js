...
import { buildCodexItems, CODEX_LOCAL_OPTS } from '../providers/codex.js';
...
export const DEFAULT_DEMO_IDS = ['claude', 'codex', 'gemini', 'antigravity', 'zai'];
...
function makeCodexDemo(context) {
  ...
  return makeSingleAccountDemo({
    ...
    plan: 'plus',
    latency: [650, 900],
    items: (now) => buildCodexItems(data, {}, { prefix: 'demo:codex', now, resetCreditExpiries }),
...
function makeZaiDemo({ clock, random }) {
  ...
  const fixtures = [
    {
      account: { name: 'key_1', key: 'demo01...keyA', proxy: 'direct' },
    ...
    {
      account: { name: 'key_2', key: 'demo02...keyB', proxy: 'http://192.0.2.10:8002' },
