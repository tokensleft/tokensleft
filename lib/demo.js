...
function makeSingleAccountDemo({ id, title, plan, windows, localModels, localFiles = true, localOpts }) {
  ...
  return {
    ...
    async fetch() {
      return { ok: true, ms: randInt(120, 900), plan, items: itemsFrom(specs, Date.now()), ...(local ? { local } : {}) };
    },
