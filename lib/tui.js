...
export function runDashboard({
  ...
  const showResetCelebration = (providerTitle, { rainbowAll = false } = {}) => {
    if (!resetCelebration) {
      resetCelebration = {
        providers: new Set(),
  ...
  const processAlerts = (state) => {
    ...
    if (unexpectedReset) {
      showResetCelebration(state.provider.title);
    }
...
export function formatHelp({ npxMode = false, width = 64 } = {}) {
...
export function formatFooter({ states, alerts, mode, resetNotice = null, width = 100 }) {
  ...
  if (resetNotice?.providers?.length > 0) {
    ...
    const summary = `{${COLOR.success}-fg}{bold}${escapeBlessed(subject)} just got ${resetText}!{/bold}{/${COLOR.success}-fg}`;
