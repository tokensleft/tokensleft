...
export function terminalProfile(env = process.env) {
  ...
  // Blessed otherwise identifies Windows Terminal as `windows-ansi` (8 colors),
  // even though Windows Terminal supports the xterm 256-color sequences.
  return env.WT_SESSION ? 'xterm-256color' : undefined;
}
...
export function runDashboard({ screenTitle, title = DASHBOARD_TITLE, subtitle = DASHBOARD_SUBTITLE, providers }) {
