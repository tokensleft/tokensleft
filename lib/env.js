...
// `--read-only` (or TOKENSLEFT_READ_ONLY in .env): never refresh tokens or
// write credentials back. Every provider consults this one definition.
export function readOnlyMode(env = process.env) {
  return /^(1|true|yes)$/i.test(String(env?.TOKENSLEFT_READ_ONLY || ''));
}
