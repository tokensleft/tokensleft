...
export function formatUsageItem(item, width) {
  ...
  if (item.exhausted) {
    lines.push(item.depletesAt
      ? `  {red-fg}{bold}⚠ ran dry ${escapeBlessed(formatRelativeTime(item.depletesAt))} — waiting for reset{/bold}{/red-fg}`
      : '  {red-fg}{bold}⚠ quota exhausted — waiting for reset{/bold}{/red-fg}');
...
export function formatUsageItemCompact(item, width) {
  ...
  const dry = item.exhausted && !item.depletesAt
    ? ' {red-fg}{bold}⚠ exhausted{/bold}{/red-fg}'
    : item.depletesAt
