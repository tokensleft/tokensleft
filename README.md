...
How many tokens do I have left? One terminal dashboard for AI subscription quota — Claude Code (Anthropic), Codex (OpenAI), Gemini CLI, GitHub Copilot, Grok CLI, Antigravity, OpenCode, and z.ai — with burn-rate forecasts, threshold alerts, automatic credential discovery + OAuth token refresh, and local per-model token/cost accounting for Claude Code (including Fable), Codex, Gemini CLI, and OpenCode.
...
| `npx tokensleft codex` | Codex (ChatGPT plan) session/weekly/model limits, reviews, credits + local token & cost table |
| `npx tokensleft gemini` | Gemini CLI Pro/Flash daily quotas + local token & cost table |
...
| `npx tokensleft opencode` | OpenCode Go plan spend vs session/weekly/monthly dollar limits + local per-model table |
...
`r` refresh all · `1`-`9` refresh one provider · `d` detail view (adds the local per-model usage tables) · `q`/`Esc` quit · arrows/PgUp/PgDn/mouse scroll.
...
|---|---|---|
| Claude Code | `~/.claude/projects/**/*.jsonl` transcripts | input/output/cache-read/cache-write split, deduped by message id |
| Codex | `~/.codex/{sessions,archived_sessions}/**/rollout-*.jsonl` | usage diffed from cumulative session totals, so repeated events and forked/resumed sessions don't double count |
| Gemini CLI | `~/.gemini/tmp/*/chats/*` session checkpoints (.json and .jsonl) | thought tokens counted as output, tool prompts as input |
| OpenCode | `opencode.db` (SQLite, read-only) | per provider/model; `$` is the cost OpenCode itself recorded per message |

For Claude Code, Codex, and Gemini the `$` column is an estimate at public API prices (cached reads/writes priced at their discounted rates) — subscription usage is prepaid, so it's there for scale, not billing. Models without published pricing show `?`.
...
- **Local usage table** (detail view, `d`) aggregates transcripts into per-model tokens and estimated cost — see [Local usage tables](#local-usage-tables-the-d-key).
