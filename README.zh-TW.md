...
- Claude Code 登入資料會從 `~/.claude/.credentials.json` 讀取；macOS 上會優先讀 login Keychain 中的 `Claude Code-credentials` 項目，首次讀取時系統會詢問授權。哪個來源有可用的 token 就用哪個，過期的舊檔不會蓋掉新的登入。Keychain 憑證只讀不寫，不會寫回。完全沒有憑證時，仍會顯示來自 Claude Code 轉錄檔的本機用量。
