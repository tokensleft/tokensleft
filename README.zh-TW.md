...
配額進度條顯示的是百分比;本地用量表告訴你這些百分比是由什麼組成的。按 `d`(詳細檢視 — 也是 `--once` 印出的內容)可看到今天 / 7 天 / 30 天 / 所有時間的逐模型明細(所有時間 = 磁碟上仍保留的紀錄),彙整自各 CLI 自己留在磁碟上的紀錄 — 首次掃描會讀完整歷史,之後只讀新增的部分,任何資料都不會離開你的機器:
