...
- Claude Code 登录信息会从 `~/.claude/.credentials.json` 读取；macOS 上会优先读 login Keychain 中的 `Claude Code-credentials` 项目，首次读取时系统会询问授权。哪个来源有可用的 token 就用哪个，过期的旧文件不会盖掉新的登录。Keychain 凭据只读不写，不会写回。完全没有凭据时，仍会显示来自 Claude Code 转录文件的本地用量。
