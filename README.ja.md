...
- Claude Code のログイン情報は `~/.claude/.credentials.json` から読み取ります。macOS では login キーチェーンの `Claude Code-credentials` 項目を使用し、初回アクセス時に macOS が許可を求めます。キーチェーンの認証情報は読み取り専用で、書き戻すことはありません。認証情報がまったくない場合でも、Claude Code のトランスクリプトによるローカル使用量は表示されます。
